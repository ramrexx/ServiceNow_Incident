=begin
 servicenow_resolve_incident.rb

 Author: Kevin Morey <kevin@redhat.com>

 Description: This method resolves a ServiceNow Incident Record via REST API
-------------------------------------------------------------------------------
...
begin
  ...
  if $evm.root['object_name'] == 'Event'
    ...
    unless $evm.root['dialog_miq_alert_description'].nil?
      ...
      body_hash['comments'] = "#{$evm.root['vmdb_object_type']}: #{@object.name} - Incident manually resolved"
    end
  end
  ...
  log(:info, "Setting custom attribute {#{@snow_incident_state} => #{servicenow_result['state']}}")
  ...
  @object.custom_set(@snow_incident_state, snow_state)
